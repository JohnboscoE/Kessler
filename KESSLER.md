...
| Vector / embedding support | **VERIFIED ABSENT** | Aug 15. `embedding` 0 hits, `hnsw` 0, `cosine` 0. All 155 `vector` hits are GraphBLAS sparse linear algebra (`GrB_Vector`, `GrBVector`, `degree_vector`) — traversal internals, unrelated to similarity search. Kessler does not need it |
| **Managed cloud is a different product** | **VERIFIED — do not use** | Aug 17. `docs.hydradb.com` advertises a hosted service at `api.hydradb.com` (sign up at `app.hydradb.com`). It is **not** the OSS graph engine. Its `/query` endpoint takes `query`, `type: knowledge\|memory`, `query_by: hybrid\|text`, `mode`, `max_results` — a document-retrieval/RAG API with no Cypher parameter, no `UNWIND`, no `algo.*` procedures, and `max_results` capped at 50. Kessler needs all four. **This resolves the vector puzzle above**: the two products share a name, and the hybrid vector+graph claim belongs to the managed service, not the OSS repo the hackathon says to build on. Self-hosting `graph-node` is mandatory |
